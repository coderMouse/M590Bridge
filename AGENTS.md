...
**V2 图片/文件、mDNS、Linux `.deb`/登录自启已完成**；Windows NSIS 已成功打包安装，task-042 的自启/跨机回归待验收；task-044 至 task-051 已完成真机验收（无文件夹/断点续传），task-052 正在接入 Linux FUSE 网络按需流。
原 task-019A（收图落盘捷径）已 **cancelled**。  
用户已明确暂停 task-042 的剩余验收；task-043 至 task-051 已完成。task-051 已验证 Linux FUSE 单文件在 GNOME Wayland Nautilus 粘贴时惰性读取、显示系统进度且内容一致；task-052 正在把该内容源接入网络 `FileRequest`，不扩展多文件/文件夹，也不复活 019A 或 task-042。
