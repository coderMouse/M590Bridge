...
**V2 图片/文件、mDNS、Linux `.deb`/登录自启已完成**；Windows NSIS 已成功打包安装，task-042 的自启/跨机回归待验收；task-044/045 已完成真机验收，task-046 文件生命周期与 task-047 Linux 关闭到托盘/图标代码完成，待一并真机验收（无文件夹/断点续传）。
原 task-019A（收图落盘捷径）已 **cancelled**。  
用户已明确暂停 task-042 的剩余验收；task-043、task-044、task-045 已完成。当前只验收 task-046/047；不复活 019A 或提前实现 Linux FUSE/多文件。
