...
- `选择文件（可多选）`：桌面原生多选对话框。
- `选择文件夹`：桌面原生目录对话框。
- 窗口拖放可一次传入多个文件和目录；目录扫描不跟随 symlink。
- 批次按条目串行传输；取消按钮取消整个批次。浏览器开发模式只保留 4MiB 单文件回退。
...
| 目录 | 手动选择/拖放或可读取目录 file_list 时作为批次目录树；OS 直接粘贴仍待 task-057/058 验收 |
...
| 4 | 是否支持 1 对多 | 不做 |
| 5 | 文件夹复制 | 手动入口已支持，file_list 已接入批次；Windows/Linux OS 直接粘贴分别待 task-057/058 验收 |
...
| v0.4 | 2026-08-17 | task-056：增加多文件/文件夹手动批次入口、双层进度与整批取消 |
