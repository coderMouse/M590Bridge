...
可操作桌面壳不把上述画板宽度当作全局 `max-width`；窗口背景、标题栏和导航始终铺满 WebView。

| 窗口宽度 | 导航 | 内容布局 |
|----------|------|----------|
| `< 640px` | 底部导航 | 单列；保留 400–460px 紧凑工具窗体验 |
| `640–1023px` | 左侧导航 | 单列内容随窗口扩展；配对表单保持可读宽度 |
| `>= 1024px` | 左侧导航 | 主面板和设置页双列；内容区最大可读宽度 1200px |

- Tauri 初始窗口：`460 × 760`；最小窗口：`400 × 600`。
- 应用根节点使用完整窗口宽高，主内容区独立滚动；标题栏与当前导航保持可见。
- 不按窗口宽度缩放字体或控件；通过重排、分栏和合理内容宽度利用空间。
...
**本机 Hub 离线（桌面壳）**：顶栏显示 `API 未连接`；琥珀色提示条按真实原因文案，而不是固定一句“正在启动”：
- 启动中：`内嵌 Hub 正在启动…`
- 端口占用：提示退出重复的 M590Bridge/`m590-daemon` 进程
- 启动失败 / 鉴权失败 / 来源拒绝：给出对应重启或入口说明
- 正式/standalone 桌面壳通过 Tauri IPC 访问内嵌 Hub（不在 WebView 里直接 `fetch http://127.0.0.1`，避免 https 页面混合内容拦截）
- 浏览器开发模式仍用 fetch，并提示手动启动独立 hub
...
4. **启动**（Linux / Windows 桌面壳显示）
   - 登录时自动启动
   - 正式安装版/standalone 桌面端可开启；依赖 Vite 的开发壳应拒绝并显示明确错误
   - Linux 写 XDG autostart；Windows 写当前用户 HKCU Run
5. **通知**
   - 同步成功通知
   - 传输完成通知
   - 断开连接通知
...
| 最近同步 | 本地短历史存储 | 可后置 |
| 暂停同步 | 暂停自动同步标志 | 是 |
| Linux / Windows 登录自启 | XDG autostart / HKCU Run；开发壳禁止开启 | V3 |
...
| v0.2 | 2026-08-10 | 设置页增加 Linux 用户登录自启开关 |
| v0.3 | 2026-08-11 | 登录自启开关扩展到 Windows；NSIS 当前用户安装待真机验收 |
