...
> 更新：2026-08-17
> 阶段：task-055 批次协议基础已完成；task-056 多文件选择与串行传输待开发
...
- [ ] **task-056** 多文件选择与批次顺序传输（依赖 task-055）
...
| 多文件批次清单协议 | **基础已完成**（task-055；type 16、路径/资源上限、wire round-trip；运行时尚未接入） |
...
1. 执行 **task-056**：多文件选择、目录扫描与串行批次传输
2. 按平台分别执行 **task-057**（Windows Explorer）和 **task-058**（Linux FUSE 目录树）
3. 完成 Linux↔Windows 多文件/文件夹端到端真机验收
4. （可选）设置页发现开关 / 本机显示名
...
> task-042 已完成 Windows 安装、自启、卸载清理与跨机回归验收；task-055 只扩展了批次
> 协议基础，运行时仍不支持断点续传、多文件或文件夹。
