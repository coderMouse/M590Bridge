...
1. task-060（进行中）：Q1/Q2 串行重开已真机通过；Q3 权限修复（FUSE `0o664`/
   `0o775`）已真机通过；Q4 角标已真机消失；**mp4/单个 pdf 替换真机通过；另一 pdf
   仍报「拼接文件时出错」→ 二轮修复（忽略旧轮在途 `FileComplete`）已提交，待真机
   复测**（txt/apk/zip 正常）
...
> 权限修复与 Q4 角标已真机通过；**mp4/pdf 粘贴替换修复分两轮：六轮（旧流在途先取消
> 再重开 + 陈旧 chunk 丢弃 + 管道重开门控）真机 mp4/单个 pdf 通过；七轮（忽略旧轮
> 在途 `FileComplete(ok)`）已提交，待真机复测（见 task-060）**。并发重开、
