...
> 更新：2026-08-17
> 阶段：task-056 多文件选择与串行传输已实现并通过本地自动化验证，待两端桌面交互/跨机复测
...
- [ ] **task-056** 多文件选择、目录扫描与串行批次传输（实现完成，待 Linux/Windows 真机验收）
...
- [ ] **task-057** Windows Explorer 多文件剪贴板粘贴（依赖 task-056）
...
| 多文件/目录手动批次 | **实现完成待真机验收**（task-056；原生多选/目录/拖放、安全扫描、串行传输、整批暂存发布、双层进度/取消） |
...
1. 完成 **task-056** Linux/Windows 桌面多选、文件夹、拖放、取消/替换/断线与跨机内容验收
2. 验收通过后按平台分别执行 **task-057**（Windows Explorer）和 **task-058**（Linux FUSE 目录树）
...
> task-042 已完成 Windows 安装、自启、卸载清理与跨机回归验收；task-056 已实现手动
> 多文件/目录批次，但尚未完成两端真机交互验收，也不包含 OS 文件剪贴板目录树或断点续传。
