...
> 更新：2026-09-01
> 版本：**0.1.3**（0.1.2 → 0.1.3，随 task-060/061/062 三项完成后的收尾。唯一版本源为根 `Cargo.toml`，见 task-059）
...
- **task-063**（优化项，已立项待实现）：含文件夹的批次不弹系统复制进度窗口 —— `FD_PROGRESSUI` 只加在非目录条目上。最小改动是把它移出 `if !entry.is_directory()`，让所有条目（含目录）都带此标志，而 `FD_FILESIZE` 与 `nFileSize*` 仍只给文件（目录无流）。**机制已定位，不是推测**。实现几率 85%：改动极小（1 行）、风险隔离好（不碰流逻辑/协议/Hub/UI）、测试覆盖充分（代码级 + task-058 真机回归清单）；15% 的不确定来自 shell 决定论（`FD_PROGRESSUI` 只是提示，最终是否显示由 Windows shell 决定，可能有其他隐藏条件）。**优先级低于功能开发**：这是优化项，task-058 目录树粘贴已真机验收通过，任何回归都比缺进度窗口严重 —— 真机验证必须先跑 task-058 回归清单，任何一项回归立即回滚。
