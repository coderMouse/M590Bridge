...
> 更新：2026-09-02
> 版本：**0.1.4**（0.1.3 → 0.1.4，随 task-063 完成。唯一版本源为根 `Cargo.toml`，见 task-059）
> 阶段：task-063 已完成（Windows 含文件夹批次显示系统复制进度窗口）
...
- [x] **task-060** 重复粘贴与替换/图标回归（Q1/Q2/Q3/Q4 均真机通过；原「目录 + 其他文件混合批次替换」已知绕行已于 2026-09-02 验证通过）
- [x] **task-061** 图片与图片文件双表示（Windows 侧验收 + Windows→Linux 图片 + 诊断收敛全部真机通过）
- [x] **task-062** Windows 批次取消按钮修复（真机通过）
- [x] **task-063** Windows 粘贴含文件夹的批次时弹出系统复制进度窗口（真机通过）
...
> 重开（发送源保留、stream offer 保留、bridge/FUSE 重开），Q1/Q2/Q3/Q4 均真机通过；
> **包括混合批次替换（目录 + 其他文件）在内的所有场景已于 2026-09-02 验证通过，
> 日常使用无需规避**。并发重开、断点续传与替换后重开仍不保证。task-061 已完成图片/
> 图片文件「双表示」粘贴（Word 贴位图 + 文件管理器贴图片文件），方案 A 接收端物化。
> task-059 已将应用版本收敛到根 workspace，并通过 Tauri Linux 实包确认 `.deb` 元数据随
> 版本号更新。
