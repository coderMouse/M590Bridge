...
> 更新：2026-08-10
> 阶段：文本+图片+V2 文件流式 + mDNS + Linux .deb + 自适应桌面 UI；**文件通道安全基线已完成**
...
1. Linux↔Windows 对 task-036 做同文件、同网络实机吞吐复测
2. Linux 登录自启（用户级、可显式启停；先创建独立 task）
3. （后续）文件失败反馈、取消/重试、会话加密与配对码随机性
4. Windows 安装包 / 开机自启
5. （可选）独立文件数据连接 / 更高吞吐调优
6. （可选）设置页发现开关 / 本机显示名
7. （可选）多文件并行 / 文件夹 / OS 文件剪贴板

> task-036 已完成本机验证；下一步先做 Linux↔Windows 实机复测，再继续 Linux 自启，一次仍只做一个 task。
