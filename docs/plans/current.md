...
1. task-060（进行中）：Q1/Q2 串行重开已真机通过；Q3 权限修复（FUSE `0o664`/
   `0o775`）已真机通过；Q4 角标已真机消失；**mp4/pdf 粘贴替换「拼接文件时出错」
   修复已实现并通过单测（旧流在途时先取消再重开 + 陈旧 chunk 丢弃 + 管道重开门控），
   待真机复测**（txt/apk/zip 正常）
...
> 权限修复与 Q4 角标已真机通过；**mp4/pdf 粘贴替换仍报「拼接文件时出错：输入/输出
> 错误」的修复已实现并通过单测（见 task-060 六轮记录），待真机复测**。并发重开、
> 断点续传与替换后重开仍不保证。task-059 已将应用版本收敛到
