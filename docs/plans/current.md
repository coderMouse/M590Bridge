...
> 更新：2026-08-14
> 阶段：task-046 文件 offer/按需传输生命周期代码完成，待 Windows↔Linux 真机验收；task-042 继续暂停
...
1. 完成 task-046，并由用户做 Windows↔Linux 文件生命周期真机复测
2. 新建 task-047：Linux 点击 X 后仅保留托盘，并统一任务栏/托盘应用图标
3. 等待用户决定是否恢复 task-042 剩余的登录自启与跨机回归验收（当前暂停）
4. （可选）独立文件数据连接 / 更高吞吐调优
5. （可选）设置页发现开关 / 本机显示名
6. （可选）多文件并行 / 文件夹 / Linux FUSE

> task-044、task-045 均已完成 Windows↔Linux 真机验收。task-046 来自后续实机生命周期反馈；Linux 窗口/图标问题排在 task-047。task-042 已确认 NSIS 可生成并安装，但剩余验收暂停且仍为 `in_progress`。
