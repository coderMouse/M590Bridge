...
1. task-060（进行中）：Q1/Q2 串行重开已真机通过；Q3 连锁卡死已加 publish-idle 兜底
   修复，待真机复测；Q3 替换 `ENOENT` 与 Q4 Nautilus「x」图标待真机排查
...
> 重开（发送源保留、stream offer 保留、bridge/FUSE 重开），Q1/Q2 已真机通过；Q3 连锁
> 卡死（已发布未请求 offer 永久阻塞本地剪贴板轮询）已加 `VIRTUAL_PUBLISH_IDLE_TIMEOUT`
> 兜底修复，待真机复测；Q3 替换 `ENOENT` 与 Q4 角标待真机排查。并发重开、断点续传与
> 替换后重开仍不保证。task-059 已将应用版本收敛到
