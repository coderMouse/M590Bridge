...
> 更新：2026-08-14
> 阶段：task-046/047 已完成真机验收；等待建立下一任务；task-042 继续暂停
...
1. 建立独立任务，处理 Windows Explorer 已开始粘贴时本机复制其它文件中断当前数据流的问题
2. 等待用户决定是否恢复 task-042 剩余的登录自启与跨机回归验收（当前暂停）
3. （可选）独立文件数据连接 / 更高吞吐调优
4. （可选）设置页发现开关 / 本机显示名
5. （可选）多文件并行 / 文件夹 / Linux FUSE

> task-044 至 task-047 均已完成 Windows↔Linux 真机验收。task-042 已确认 NSIS 可生成并安装，但剩余验收暂停且仍为 `in_progress`。
