...
- [x] 目录树可浏览，文件按需读取，内容和相对路径一致。
- [x] 生命周期清理和单文件回归通过；Linux 真机结果已记录。
- [x] 不因恶意清单创建绝对路径或路径穿越。

## 下一步

- 本 task 已完成。若继续支持同一份已消费 clipboard offer 直接多次 `Ctrl+V`，另建协议
  生命周期任务，不在本 task 内扩展 wire 语义。
...
- 2026-08-19：用户确认 task-058 真机测试通过。Linux GNOME Wayland + Nautilus 已覆盖
  含大文件批次、嵌套/空目录、空文件、取消、替换、断线及重新发送同批输入后再次粘贴；
  内容和相对路径一致，task-058 正式完成。
...
- Linux GNOME Wayland + Nautilus 跨机最终验收：用户确认 task-058 通过；含大文件批次、
  嵌套/空目录、空文件及取消、替换、断线、重新发送后重复粘贴均通过。

## 文档影响检查

- 已更新：本 task、当前计划、Agent 当前阶段与项目说明，补充首块后停住的真机证据、
  活跃流剪贴板所有权轮询暂停和本轮真实验证结果。
- 已更新：项目结构图和常用命令，将 Linux FUSE tree 从“真机待验收”改为已通过。
- 无需更新：协议 wire 字段、Hub HTTP API、UI 交互、Windows OLE、安装器与自启均未改变。
- 已更新：本 task、当前计划、Agent入口与项目说明，记录最终真机验收和完成状态；历史
  产物记录、协议版本和构建命令未改变。

## 风险 / blocker

- blocker：无；task-058 自动化、本地真实挂载及 Linux GNOME Wayland + Nautilus 跨机
  验收均已完成。
