...
- [x] 加入一个无法完成配对的地址并保持自动重连开启，约 30 秒后 UI 显示超时错误，不再持续配对。
- [x] 超时后无需先断开，第一次点击“连接对端”即可启动新尝试。
- [x] 配对中点击“断开 / 重置”，接口返回后第一次点击“连接对端”不再出现 bridge already running。
- [x] 创建方无对端时可持续等待；对端连入但不完成握手时约 30 秒报错。
...
- Linux↔Windows 真机复测：用户确认通过；初次总超时、超时后直接重连、断开后首次重连、创建方等待/握手超时及已连接后自动重连均验收完成。
...
- 无；本 task 的跨平台交互已完成真机验收。

## 下一步

- task-049 已完成；后续根据用户优先级建立新 task，task-042 继续保持暂停。
