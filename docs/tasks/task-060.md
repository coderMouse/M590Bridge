...
`in_progress`（Q1/Q2、Q3 权限、Q4 角标均真机通过；七轮修复（忽略旧轮在途 FileComplete）后 mp4/多个 pdf 替换用户已确认通过；**八轮「目录 + mp4」批次替换的条目失败隔离真机复测仍有问题，已按用户决定回滚到七轮版本 8593776；日常规避：文件夹与其他文件分开复制**）
...
- **已回滚**：八轮（hub.rs 条目失败隔离）真机复测仍有问题，已回退到七轮版本
  （8593776）。当前 task-060 以七轮通过状态收尾；日常使用以「文件夹与其他文件分开
  复制」规避批次替换问题。
