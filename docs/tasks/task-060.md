...
`completed`（2026-09-01）—— Q1/Q2/Q3/Q4 均真机通过，七轮修复（忽略旧轮在途 FileComplete）后 mp4/多个 pdf 替换通过。**遗留一个已知绕行**：「目录 + mp4」批次替换的条目失败隔离（八轮）真机复测仍有问题，已按用户决定回滚到七轮版本 `8593776`；日常规避：文件夹与其他文件分开复制。
...
无。task-060 已完成（2026-09-01）。Q1/Q2/Q3/Q4 均已真机通过。遗留「目录 + 其他文件混合批次替换」已知绕行（分开复制），若后续需要完整支持另开新 task。
